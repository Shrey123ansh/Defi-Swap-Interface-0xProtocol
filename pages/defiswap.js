...
export default function Defiswap() {
  ...
  const config = {
    apiKey: "aA4DG7Zv7FfyA7ghw0SrwXkMzUwSvbxL",
  ...
  async function getPrice() {
    ...
    const priceParams = new URLSearchParams({
      ...
      sellToken: faddr, //ETH
      buyToken: taddr, //DAI
      sellAmount: amount, // Note that the WETH token uses 18 decimal places, so `sellAmount` is `100 * 10^18`.
      taker: userWallet, //Address that will make the trade
    });
    ...
    let swapOrders = await sources.json();
    console.log(swapOrders);
    try {
      await swapOrders.route.fills.find((item) => {
  ...
  async function swapit() {
    ...
    var proxy = quote.issues.allowance.spender;
        ...
        <Grid sm={5}>
          <Card variant="bordered">
            <Card.Header>
              <Row>
                <Col>
                  <img src="web3profile.png" width={"100%"} />
                </Col>
                {/* <Col>
                  <Avatar
                    src="profile.jpg"
